              ...
              <ProductColumn
                ...
                empty={
                  <p className="text-xs leading-relaxed text-white/40">
                    ...
                    하면 같은 검색어로 토스 가격이 여기에 세로로 표시됩니다.
                  </p>
                }
              />
            </div>
...
function ProductColumn({
  ...
  if (!products.length && !empty) return null
  ...
  return (
    <div className="space-y-2">
      ...
      {products.length ? (
        <ul className={list ? 'flex flex-col gap-2' : 'grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5'}>
          {products.map((p) => {
            const key = `${p.rank}-${p.title}`
            return (
              <li
                ...
                className={
                  list
                    ? 'flex min-w-0 gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-2.5'
                    : 'flex min-w-0 flex-col rounded-2xl border border-white/10 bg-white/[0.03] p-2.5'
                }
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                ...
                <div className={list ? 'min-w-0 flex-1' : ''}>
                  <p className={`line-clamp-2 text-xs leading-snug ${list ? '' : 'mt-2 min-h-[2.5rem]'}`}>
                    {p.title}
                  </p>
        ...
        <div className="rounded-2xl border border-dashed border-white/10 px-4 py-6">{empty}</div>
      )}
    </div>
  )
}
