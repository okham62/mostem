...
export function PlatformCard({
  ...
  return (
    <button
      ...
      disabled={isDisabled}
      className={cn(
        'relative flex flex-col items-center gap-2.5 rounded-xl border-2 p-4 text-center transition-all duration-150',
        isSelected
          ? 'border-brand bg-brand/5 dark:bg-brand/10'
          : 'border-[var(--card-border)] bg-[var(--card-bg)] hover:border-[var(--muted)] hover:bg-[var(--muted-bg)]',
    ...
    >
      ...
      {!isConnected && (
        <Link2Off className="absolute right-2 top-2 h-3.5 w-3.5 text-[var(--muted)]" />
      )}
      ...
      {!isConnected && (
        <span className="text-[10px] text-[var(--muted)]">미연결</span>
      )}
    </button>
  )
}
